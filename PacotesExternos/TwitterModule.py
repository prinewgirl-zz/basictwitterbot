...
class ManageTwitter:
    ...
    def search(self, query, language = 'en', items = 1):
        if language == None:
            result = tweepy.Cursor(self.api.search, q = query).items(items)
        else:
            result = tweepy.Cursor(self.api.search, q = query, 
                               lang = language).items(items)
        return result
